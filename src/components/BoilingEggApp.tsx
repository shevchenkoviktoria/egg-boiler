...
const BoilingEggApp: React.FC = () => {
    ...
    return (
        <Container
            maxWidth="xs"
            sx={{ textAlign: "center", padding: 4, backgroundColor: "#FFFAE5", borderRadius: 3 }}
        >
            <Typography variant="h4" gutterBottom>
                🥚 Cute Egg Boiler
            </Typography>
            <motion.div animate={{ rotate: isBoiling ? 360 : 0 }} transition={{ duration: 2, repeat: Infinity }}>
            ...
            {isBoiling && timeLeft !== null ? (
                <>
                    ...
                    <CircularProgress variant="determinate" value={(timeLeft / 10) * 100} sx={{ marginTop: 2 }} />
                </>
            ) : (
                <>
                    <Typography variant="h6" mt={2}>
                    ...
                    {eggTypes.map((egg) => (
                        <Button key={egg.time} variant="contained" sx={{ m: 1 }} onClick={() => startBoiling(egg.time)}>
                            {egg.label}
                        </Button>
                    ))}
                </>
